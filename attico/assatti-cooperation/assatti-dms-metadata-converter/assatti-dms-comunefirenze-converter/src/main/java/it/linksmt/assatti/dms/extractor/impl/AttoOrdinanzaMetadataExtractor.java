...
public class AttoOrdinanzaMetadataExtractor implements DmsMetadataExtractor {
	...
	public DmsPermanentProperties extractMetadataByTipoAtto(Atto atto, DocumentoPdf documentoPrincipale, ApplicationContext appContext) {
		...
		MetadataCommonUtil.addAutoreByCodiceIncarico(retVal, atto, appContext, 
				ConfigurazioneTaskEnum.ORD_RESPONSABILE_PROPONENTE.getCodice());
		
		// Proprietà specifiche delle ordinanze
		...
		if (atto.getEmananteProfilo() != null) {
			...
			documentProperties.put(DmsComuneFirenzeConstants.FIRMATARIO_ORDINANZA, firmatario);
			if (!StringUtil.isNull(atto.getDescrizioneQualificaEmanante())) {
				documentProperties.put(DmsComuneFirenzeConstants.RUOLO_FIRMATARIO_ORDINANZA, atto.getDescrizioneQualificaEmanante());
			}
		}
