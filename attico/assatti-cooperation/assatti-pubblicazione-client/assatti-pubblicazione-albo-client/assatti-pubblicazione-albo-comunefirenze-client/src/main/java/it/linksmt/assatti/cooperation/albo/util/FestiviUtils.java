...
public class FestiviUtils {
	...
	public Date aggiungiEnneGiorniLavorativi(Date dataPartenza, int numeroGiorni) throws ParseException {
		...
		List<Date> festivi = getFestiviProssimiNAnni((numeroGiorni / 365) + 2);
	...
	private List<Date> getFestiviProssimiNAnni(int n) throws ParseException {
		...
		for (int anno = questanno; anno < questanno + n + 1; anno++) {
			festivi.addAll(getFestiviByAnno(anno));
		}
		return festivi;
	}

	private List<Date> getFestiviByAnno(int anno) throws ParseException {
		List<Date> festivi = new ArrayList<Date>();
		...
		List<Date> festiviFissi = getFestiviFissiByAnno(anno);
		...
		festivi.add(getPasquettaByAnno(anno));
		return festivi;
	}

	private List<Date> getFestiviFissiByAnno(int anno) throws ParseException {
		DateFormat df = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
		List<Date> festivi = new ArrayList<Date>();
	...
	private Date getPasquettaByAnno(int anno) throws ParseException {
		DateFormat df = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
		int a = anno % 19, b = anno / 100, c = anno % 100, d = b / 4, e = b % 4, g = (8 * b + 13) / 25, h = (19 * a + b - d - g + 15) % 30, j = c / 4, k = c % 4, m = (a + 11 * h) / 319,
				r = (2 * e + 2 * j - k - h + m + 32) % 7, mese = (h - m + r + 90) / 25, giorno = (h - m + r + mese + 19) % 32;
