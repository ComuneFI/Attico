  ...
  angular.module('mgcrea.ngStrap.datepicker', [ 'mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.helpers.dateFormatter', 'mgcrea.ngStrap.tooltip' ]).provider('$datepicker', function() {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        angular.forEach([ 'template', 'templateUrl', 'controller', 'controllerAs', 'maxView', 'placement', 'container', 'delay', 'trigger', 'html', 'animation', 'autoclose', 'dateType', 'dateFormat', 'timezone', 'modelDateFormat', 'dayFormat', 'strictFormat', 'startWeek', 'startDate', 'useNative', 'lang', 'startView', 'minView', 'maxView', 'iconLeft', 'iconRight', 'daysOfWeekDisabled', 'id', 'prefixClass', 'prefixEvent' ], function(key) {
          if (angular.isDefined(attr[key])) options[key] = attr[key];
        });
        var falseValueRegExp = /^(false|0|)$/i;
      ...
      return function(picker) {
        ...
        var views = [ {
          ...
          build: function() {
            ...
            if(!options.maxView || options.maxView > 1){
            	scope.title = formatDate(month, options.yearTitleFormat);
            }else{
            	scope.title = 'Indietro';
            }
        ...
        var myViews = options.minView ? Array.prototype.slice.call(views, options.minView) : views;
        if(options.maxView){
      	  for(var i = 0; i< 2 - options.maxView; i++){
      		  Array.prototype.pop.call(myViews);
      	  }
        }
        return {
          views: myViews,
